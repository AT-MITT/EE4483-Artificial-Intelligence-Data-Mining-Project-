...
MODEL_PATH = '/home/users/ntu/po0001yi/scratch/AIDM/best_cifar10_resnet50.pth'
...
model.fc = nn.Linear(model.fc.in_features, CIFAR_NUM_CLASSES)
...
# Remove incompatible final layer
...
print(f"Loaded partial weights (fc layer excluded) from: {MODEL_PATH}")
...
image_files = [f"{i}.png" for i in range(len(test_data))]
submission = pd.DataFrame({
    "id": image_files,
    "label": all_preds
})
submission.to_csv(os.path.join(OUTPUT_DIR, "submission_resnet50_CIFAR.csv"), index=False)
print(f"Submission file saved to: {os.path.join(OUTPUT_DIR, 'submission_resnet50_CIFAR.csv')}")
...
conf_matrix_path = os.path.join(OUTPUT_DIR, "resnet50_cifar10_test_confusion_matrix.png")
