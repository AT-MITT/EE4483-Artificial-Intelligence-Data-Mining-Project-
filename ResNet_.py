...
model_cifar.fc = nn.Linear(in_features, NUM_CLASSES)
model = model_cifar.to(DEVICE)
...
# Modify the final fully connected layer to match the number of classes (CIFAR-10)
in_features = model_imbalanced.fc.in_features  # Get the number of input features of the fully connected layer
model_imbalanced.fc = nn.Linear(in_features, CIFAR_NUM_CLASSES)  # Replace the fc layer
model_imbalanced = model_imbalanced.to(DEVICE)  # Move model to device
